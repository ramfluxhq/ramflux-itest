...
pub struct LoadgenConfig {
    ...
    pub gateway_quic_addr: std::net::SocketAddr,
    pub gateway_server_name: String,
    pub gateway_ca_cert: PathBuf,
    pub federation_mesh_endpoints: Vec<String>,
    pub federation_server_name: String,
    pub federation_tls: ramflux_transport::MeshTlsConfig,
    pub federation_peer_ca_pems: Vec<String>,
    pub source_node_id: String,
    pub target_node_id: String,
    pub request_timeout: Duration,
    pub gateway_metrics_url: Option<String>,
    pub router_metrics_url: Option<String>,
...
pub async fn run_from_env() -> Result<serde_json::Value, Box<dyn std::error::Error>> {
    let config = LoadgenConfig::from_env()?;
    let artifact = run_loadgen(config).await?;
...
impl LoadgenConfig {
    ...
    pub fn gateway_quic_envelope(
        ...
        Ok(Self {
            target: LoadgenTarget::GatewayQuicEnvelope,
            mode: SignMode::Pregenerated,
            total: total.max(1),
            connections: connections.max(1),
            inflight_per_connection: inflight_per_connection.max(1),
            cardinality: cardinality.max(1),
            created_at: i64::try_from(current_epoch_seconds())?,
            ttl: 3_600,
            device_signing_seed: decode_seed_env(
                "RAMFLUX_LOADGEN_DEVICE_SIGNING_SEED_B64URL",
                DEFAULT_DEVICE_SIGNING_SEED,
            )?,
            gateway_quic_addr,
            gateway_server_name,
            gateway_ca_cert,
            federation_mesh_endpoints: Vec::new(),
            federation_server_name: DEFAULT_FEDERATION_SERVER_NAME.to_owned(),
            federation_tls: ramflux_transport::MeshTlsConfig {
                ca_cert: PathBuf::from(DEFAULT_FEDERATION_SOURCE_CA_CERT),
                service_cert: PathBuf::from(DEFAULT_FEDERATION_SOURCE_CERT),
                service_key: PathBuf::from(DEFAULT_FEDERATION_SOURCE_KEY),
            },
            federation_peer_ca_pems: Vec::new(),
            source_node_id: DEFAULT_SOURCE_NODE_ID.to_owned(),
            target_node_id: DEFAULT_TARGET_NODE_ID.to_owned(),
