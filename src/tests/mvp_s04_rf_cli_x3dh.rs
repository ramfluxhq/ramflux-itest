...
#[cfg(feature = "realnet")]
#[test]
fn mvp_s4_realnet_rf_cli_account_dm_group() -> Result<(), Box<dyn std::error::Error>> {
    ...
    runtime.block_on(async {
        Box::pin(mvp_s4_assert_rf_cli_flow(gateway_quic_addr, &ca_cert, &realnet.gateway_url))
            .await?;
...
#[cfg(feature = "realnet")]
#[test]
fn mvp_s4b_realnet_x3dh_cli_dm_bootstrap_ratchet() -> Result<(), Box<dyn std::error::Error>> {
    if std::env::var("RAMFLUX_ITEST_REALNET").as_deref() != Ok("1") {
        eprintln!("skipping realnet full test; set RAMFLUX_ITEST_REALNET=1");
        return Ok(());
    }
    ...
    let runtime = tokio::runtime::Builder::new_current_thread().enable_all().build()?;
    runtime.block_on(async {
        Box::pin(mvp_s4_assert_rf_cli_second_x3dh_bootstrap(
            gateway_quic_addr,
            &ca_cert,
            &realnet.gateway_url,
        ))
