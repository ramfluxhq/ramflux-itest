...
fn mvp_s53_realnet_quic_ingress_group_commit_compare() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let off = run_s53_stage(&plan, false)?;
    let on = run_s53_stage(&plan, true)?;
    let comparison = build_s53_comparison(&plan, &off, &on);
    let artifact = s53_artifact_path("mvp_s53_quic_ingress_group_commit_compare_latest.json")?;
...
fn run_s53_stage(
    ...
    if let Some(object) = artifact.as_object_mut() {
        ...
        object.insert("router_group_commit".to_owned(), json!(group_commit));
    }
...
fn build_s53_comparison(
    ...
    json!({
        "schema": "ramflux.itest.mvp_s53.quic_ingress_group_commit_compare.v1",
